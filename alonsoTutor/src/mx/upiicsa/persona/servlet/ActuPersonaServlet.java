...
@WebServlet("/ActuPersonaServlet")
public class ActuPersonaServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	public ActuPersonaServlet() {
		super();
	}
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (nombre.isEmpty() || primerApellido.isEmpty() || segundoApellido.isEmpty() || curp.isEmpty() || genero.isEmpty() || tipoSangre.isEmpty()) {
			rd = request.getRequestDispatcher("ActualizaPersona.jsp");
			codigo = -1;
			...
			int resultado = personabs.actualizaPersona(persona);
			System.out.println("--->resultado" + resultado);
			
			if (resultado == 1) {
				System.out.println("--->Dentro del if" + resultado);
				rd = request.getRequestDispatcher("exitoActuPersona.jsp");
				codigo = 1;
				misesion.setAttribute("codigo", codigo);
				rd.forward(request, response);
			}
